...
-- Drop and recreate the 4-param bootstrap overload with inlined property type breakdown
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- Inlined property type breakdown (no external function call)
    'propertyTypeBreakdown', CASE WHEN target_brokerage IS NOT NULL THEN COALESCE((
      ...
      SELECT jsonb_agg(
        jsonb_build_object(
          'property_type', ptb.property_type,
          'mentions', ptb.mentions,
          'rank', RANK() OVER (ORDER BY ptb.mentions DESC),
          'total_brokerages', ptb.total_brokerages
        ) ORDER BY ptb.mentions DESC
      )
      FROM ptb_counts ptb
    ), '[]'::jsonb) ELSE '[]'::jsonb END,
...
-- Drop and recreate the 5-param bootstrap overload (with state_filter) also with inlined property type breakdown
CREATE OR REPLACE FUNCTION public.get_cre_dashboard_bootstrap(
  target_brokerage text DEFAULT NULL,
  market_filter text DEFAULT NULL,
  property_type_filter text DEFAULT NULL,
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- Market rankings filtered by state
    'marketRankings', CASE WHEN target_brokerage IS NOT NULL THEN COALESCE((
      WITH brokerages_per_market AS (
        SELECT bmr2.market, COUNT(*)::int AS cnt
        FROM brokerage_market_rankings bmr2
    ...
    ELSE '{}'::jsonb END,
    -- Primary markets filtered by state
    'primaryMarkets', CASE WHEN target_brokerage IS NOT NULL THEN COALESCE((
      SELECT jsonb_agg(pm ORDER BY pm)
      FROM (
