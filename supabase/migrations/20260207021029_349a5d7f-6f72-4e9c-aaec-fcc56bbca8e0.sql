...
DROP FUNCTION IF EXISTS public.get_property_type_breakdown(text, text);
...
AS $$
  WITH all_brokerage_property_counts AS (
    -- Count mentions per brokerage per property type (with optional market filter)
    SELECT 
    ...
    GROUP BY COALESCE(e.brokerage, e.name), p.property_type
  ),
  ranked_brokerages AS (
    -- Rank brokerages within each property type
    SELECT 
      brokerage,
      property_type,
      mentions,
