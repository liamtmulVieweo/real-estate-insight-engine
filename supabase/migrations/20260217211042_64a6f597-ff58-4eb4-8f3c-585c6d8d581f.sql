...
CREATE OR REPLACE FUNCTION public.get_source_attribution_comparison(target_brokerage text)
 RETURNS TABLE(domain text, target_rank bigint, target_pct double precision, peer_avg_rank double precision, peer_avg_pct double precision, diff_pct double precision, category text)
 LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION public.get_source_attribution_vs_competitor(target_brokerage text, competitor_brokerage text)
